...
getRelatedHashs <- function(buzzWords) {
  ...
  lrelated <- lapply(buzzWords, function(x) {
    lrel_syn <- lapply(x[1:length(x)], function (y) {
      while (TRUE) {
        tweets <- try(suppressWarnings(searchTwitter(y, n = 100, retryOnRateLimit = 250)))
...
for (i in names(countedSumSorted_df)) {
  CairoPDF(paste0("PDF/", i, format(time, "%m%d_%H%M"),"_hashtag_plots.pdf"), width = 20, height = 10)
